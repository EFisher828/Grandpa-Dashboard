...
const buildGaugeWindChart = (stationSpeed,stationGust,stationDirection,stationMaxGust) => {
	...
	const gaugeOptions = {
	    ...
	    subtitle: {
	        text: `Station: Grandfather Mountain (GRANDFATHR)<br/>24-Hour High: ${stationMaxGust} mph`
	    },
	    pane: {
	        size: '90%',
	        startAngle: -180,
...
const getMax = (stationSpeed,stationDirection,stationGust) => {
	let url = `https://api.climate.ncsu.edu/data?loc=GRANDFATHR&var=gustspeedmax&start=-24%20hours&end=now&int=1%20DAY&obtype=O&output=json&attr=location,datetime,var,value,value_accum,unit,score,nettype,paramtype,obtype,obnum,obtime&hash=8de315acb7cf795cf0e5a4b9d351caa2efd2a922`
	console.log(url)
			...
			try {
				...
				if (stationLength == 1){
					for(let key in stationObj){
						const stationMaxGustHold = stationObj[key].gustspeedmax.value
...
/*
	...
	Highcharts.chart('windSeriesContainer', {
	    ...
	    subtitle: {
	        text: `Station: Grandfather Mountain (GRANDFATHR)<br/>24-Hour High: ${allMax} mph`
	    },
