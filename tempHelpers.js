const buildGaugeTempChart = (stationTemp,stationFeels,stationMax,stationMin) => {
	...
	let speedGauge = Highcharts.chart('tempCurrentContainer', {
		...
		subtitle: {
				text: `Station: Mount GRANDFATHRhell (GRANDFATHR)<br/>24-Hour High: ${stationMax}°F     Low: ${stationMin}°F`
		},
		pane: {
				size: '90%',
				startAngle: -180,
		...
		series: [{
				...
				dataLabels: {
						format: '<div style="text-align:center"><span style="font-size:25px;color:black">{y}&deg;F</span><br/></div>'
				}
		}]
	})
	getCurrentWind()
...
const getMaxMin = (stationTemp) => {
	let url = `https://api.climate.ncsu.edu/data?loc=GRANDFATHR&var=airtempmax,airtempmin&start=-24%20hours&end=now&int=1%20DAY&obtype=O&output=json&attr=location,datetime,var,value,value_accum,unit,score,nettype,paramtype,obtype,obnum,obtime&hash=8de315acb7cf795cf0e5a4b9d351caa2efd2a922`
	console.log(url)
		...
		if(xhr.readyState === XMLHttpRequest.DONE){
			...
			if (stationLength == 1){
				for(let key in stationObj){
					const stationMaxHold = stationObj[key].airtempmax.value
					const stationMinHold = stationObj[key].airtempmin.value
			...
			buildGaugeTempChart(stationTemp,stationMax,stationMin)
			try {
			} catch(err) {}
		}
	}
...
const getFeels = (stationTemp,feelsVar) => {
	let url = `https://api.climate.ncsu.edu/data?loc=GRANDFATHR&var=${feelsVar}&start=-20%20minutes&end=now&obtype=O&output=json&attr=location,datetime,var,value,value_accum,unit,score,nettype,paramtype,obtype,obnum,obtime&hash=8de315acb7cf795cf0e5a4b9d351caa2efd2a922`
	console.log(url)
	...
	xhr.onreadystatechange = function() {
		if(xhr.readyState === XMLHttpRequest.DONE){
			...
			try {
				...
				buildTempChart(tempList,timeList,stationMax,stationMin)
			} catch(err) {}
		}
	}
	xhr.open("GET",url)
...
const getOne = (tempList,feels,timeList) => {
	let url = `https://api.climate.ncsu.edu/data?loc=GRANDFATHR&var=${feels}&start=-24%20hours&end=now&int=30%20MINUTE&obtype=O&output=json&attr=location,datetime,var,value,value_accum,unit,score,nettype,paramtype,obtype,obnum,obtime&hash=8de315acb7cf795cf0e5a4b9d351caa2efd2a922`
	console.log(url)
